...
They both have the same explanation for dealing with NAs via na.rm:
...
#### 5. Two dimensional plots reveal outliers that are not visible in one dimensional plots. For example, some points in the plot below have an unusual combination of x and y values, which makes the points outliers even though their x and y values appear normal when examined separately.
      ...
      Binning a variable here won't help since the abnormality might be eihter with x or y variable, so it is better to investigate with a sccatter plot. The problem is not 1 dimensional. It is not linearlt separable with 1 dimension.
```{r}
ggplot(data = diamonds) +
